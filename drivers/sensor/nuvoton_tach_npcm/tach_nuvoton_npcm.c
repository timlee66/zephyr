...
#include <dt-bindings/sensor/npcm4xx_tach.h>
...
static inline void tach_npcm_start(const struct device *dev)
{
	 ...
	 */
	inst->TMCTRL |= BIT(NPCM4XX_TMCTRL_TAEN);

	/* Enable input debounce logic into TA pin. */
	inst->TCFG |= BIT(NPCM4XX_TCFG_TADBEN);

	/* Select clock source of timer 1 from no clock and start to count. */
	SET_FIELD(inst->TCKC, NPCM4XX_TCKC_C1CSEL_FIELD, data->input_clk == 32768
...
static inline void tach_npcm_clear_underflow_flag(const struct device *dev)
{
	...
	inst->TECLR = BIT(NPCM4XX_TECLR_TCCLR);
}
...
static inline bool tach_npcm_is_captured(const struct device *dev)
{
	...
	LOG_DBG("tach is captured %d", IS_BIT_SET(inst->TECTRL, NPCM4XX_TECTRL_TAPND));

	/*
	 * In mode 5, the flag TAPND indicates a input captured on TAn transition.
	 */
	return IS_BIT_SET(inst->TECTRL, NPCM4XX_TECTRL_TAPND);
}
...
static inline void tach_npcm_clear_captured_flag(const struct device *dev)
{
	...
	inst->TECLR = BIT(NPCM4XX_TECLR_TACLR);
}
...
static int tach_npcm_configure(const struct device *dev)
{
	...
	SET_FIELD(inst->TMCTRL, NPCM4XX_TMCTRL_MDSEL_FIELD, NPCM_TACH_MDSEL);

	/* Configure clock module and its frequency of tachometer */
	if (config->sample_clk == 0) {
		return -EINVAL;
	} else if (data->input_clk == 32768) {
		/* Enable low power mode */
		inst->TCKC |= BIT(NPCM4XX_TCKC_LOW_PWR);
		if (config->sample_clk != data->input_clk) {
			LOG_ERR("tach operate freq is %d not fixed to 32kHz",
				data->input_clk);
			return -EINVAL;
		}
	} else {
		...
		if (data->input_clk < config->sample_clk) {
			LOG_ERR("tach operate freq exceeds APB1 clock");
			return -EINVAL;
		}
	...
	if (config->pin_static ||
		...
		SET_FIELD(inst->TCFG, NPCM4XX_TCFG_MFT_IN_SEL, config->tach_channel);
	} else {
		/* select pin to sample */
		SET_FIELD(inst->TCFG, NPCM4XX_TCFG_MFT_IN_SEL, config->pin_select);
	}
...
/* TACH api functions */
static int tach_npcm_sample_fetch(const struct device *dev, enum sensor_channel chan)
{
	ARG_UNUSED(chan);
	struct tach_npcm_data *const data = dev->data;
